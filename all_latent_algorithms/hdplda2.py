...
class HDPLDA:
    ...
    def worddist(self):
        ...
        dictionary = [DefaultDict(self.beta / self.n_k[k]).update(
    ...
    def dump(self, disp_x=True):
        print len(self.x_ji[0]), len(self.x_ji)
        print len(self.t_ji[0]), len(self.t_ji)
        print len(self.k_jt[0]), len(self.k_jt)
        print len(self.n_jt[0]), len(self.n_jt)
...
def main():
    ...
    parser.add_option("-s", dest="stopwords", type="int", help="0=exclude stop words, 1=include stop words", default=0)
    ...
    # corpus = array of arrays, each array is the words in the documents
    voca = vocabulary.Vocabulary(options.stopwords == 0)
    docs = [voca.doc_to_ids(doc) for doc in corpus]

    if options.df > 0: docs = voca.cut_low_freq(docs, options.df)
    # voca.size() ==  length of tags
    print docs

    hdplda = HDPLDA(options.alpha, options.gamma, options.beta, docs, 154)
    print "corpus=%d words=%d alpha=%.3f gamma=%.3f beta=%.3f stopwords=%d" % (
    len(corpus), len(voca.vocas), options.alpha, options.gamma, options.beta, options.stopwords)
    # hdplda.dump()
    ...
    print hdplda.worddist()[:5]
    print
    print
    print hdplda.docdist()[:5]
    print len(hdplda.worddist())
    print hdplda.docdist().shape
    hdplda.dump()
    temp = []
    for d in hdplda.worddist():
        temp.append(d.values())
    print temp[0]
