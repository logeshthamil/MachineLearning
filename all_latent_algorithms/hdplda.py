...
class HDPLDA:
    def __init__(self, alpha, gamma, base, docs, V):
        ...
        self.t_ji = [numpy.zeros(len(x_i), dtype=int) - 1 for x_i in
                     docs]  # table for each document and term (without assignment)
...
def main():
    ...
    print "corpus=%d words=%d alpha=%.3f gamma=%.3f base=%.3f stopwords=%d" % (
        len(corpus), len(voca.vocas), options.alpha, options.gamma, options.base, options.stopwords)
    # hdplda.dump()
